...
public class Server implements Runnable {
	...
	int port;
	Map<ClientHandler, Thread> handlerThreads; //keeping track of clients
...
public synchronized void addLine(String message, String name, boolean isStatus) throws IOException {
	String timestamp = getCurrentTime();
	String messageDelim = (isStatus) ? " " : ": ";
	String broadcast = timestamp + " - " + name + messageDelim + message;
	for (ClientHandler clientHandler : this.handlerThreads.keySet()) { 
		clientHandler.writer(broadcast);
	}
}
// Shows the time
...
public void close(ClientHandler clientHandler) throws InterruptedException, IOException {
	log.info("Client {} ({}) has ended the connection.", 
			clientHandler.getUser(), clientHandler.getSocket().getRemoteSocketAddress());
	clientHandler.close();
}
